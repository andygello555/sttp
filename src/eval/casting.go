...
func Cast(symbol *data.Symbol, t data.Type) (err error, cast *data.Symbol) {
	// If the entry in the matrix points to e then we will return a CannotCast error.
	if &castTable[symbol.Type][t] == &e {
		return errors.CannotCast.Errorf(symbol.Type.String(), t.String()), nil
	}
	// Otherwise, we return the result of the cast method.
	return castTable[symbol.Type][t](symbol)
}
