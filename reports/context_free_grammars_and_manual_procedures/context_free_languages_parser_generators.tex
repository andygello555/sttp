...
\begin{enumerate}
    ...
    \end{enumerate}
    \item \textbf{Context-free}: discussed in section \hyperref[sec:features-context-free]{1.3}
...
For a `human operated' derivation, the style shown in fig. 4 seems easier to digest. This is because as humans, we can lookahead in the input expression and decide which derivation step to apply. This makes manual parsing for a given grammar trivial, for example given the grammar:
...
There are ways to optimise top-down parsing such as restructuring the grammar to be accepted by an LL(1) parser. The grammar in fig. 7 is the grammar in fig. 6 restructured to remove all left-recursion.
...
\begin{figure}[H]
    \begin{center}
        ...
        \end{forest}
    \end{center}
    \cprotect\caption{The unique derivation of \verb|{"a": 0}| after restructuring the grammar in fig. 5 to be an LL(1) grammar}
\end{figure}

As you can see this is quite an involved process which results in a grammar which is very different to the grammar that was specified in \hyperref[fig:5]{fig. 5}. This confusing grammar and the difficulty of construction is often why top-down parser generators are avoided when defining the grammar for a programming language.
...
Along with those actions we also define a \textbf{lookahead} variable which defines the next available token in the input. This is necessary to parse the grammar in fig. 11. For example, if the symbol $S$ was on the stack we might need to use the lookahead to see if the next token is ``$]$" or ``$=$" to avoid reducing $S$ into $V$.\textsuperscript{\cite{johnson_zelenski_2007}\cite{knuth_1965}}

Different amount of lookahead denote different subsets of grammars. For instance, the grammar defined in fig. 11 is an $LR(1)$ grammar, where there is $1$ token of lookahead.
...
% https://www.cs.ru.nl/~herman/onderwijs/2016TnA/lecture7.pdf
% https://chomsky.info/wp-content/uploads/195609-.pdf
% CS3470 Lecture notes
% https://archive.org/details/recursiveprogram0000burg
% https://archive.org/details/compilers00ahoa
% https://www.sciencedirect.com/science/article/pii/S0019995870904468?via%3Dihub
